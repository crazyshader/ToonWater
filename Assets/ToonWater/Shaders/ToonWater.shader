...
Shader "SP/Water/ToonWater"
{
	Properties
    {
		...
		[Header(Depth)]
		_DepthScale("Depth Scale", Range(0,100)) = 1
		...
		[Header(Foam)]
		_FoamSpeed ("Foam Speed", Range (0, 1.0)) = 0.5
		_FoamStrength ("Foam Strength", Range (0.1, 100.0)) = 0.5
		_FoamGradientMap ("Foam Gradient ", 2D) = "white" {}
    }
    ...
    SubShader
    {
		...
		Pass
        {
            ...
            fixed4 frag (v2f i) : COLOR
            {
				...
				float intensityFactor = 1 - saturate(depth / _FoamStrength);  	
				//intensityFactor = 1;
				half3 foamGradient =  tex2D(_FoamGradientMap, float2(intensityFactor - _Time.y * _FoamSpeed, 0));
				half foamLightIntensity = saturate((_WorldSpaceLightPos0.y + 0.2) * 4);
				finalColor.rgb += foamGradient * intensityFactor * foamLightIntensity;
