...
Shader "SP/Water/V2/ToonWater"
{
	Properties
    {
		[Header(Mode)]
		[Toggle(_REALTIME_MODE)] _Realtime_Mode("Use RealTime Mode?", float) = 0
	...
	float GetWaterDepth(v2f i)
	{
		...
		return saturate (_DepthScale * abs(sceneZ-partZ));
	}
    ...
    SubShader
    {
		...
		Pass
        {
			...
			#pragma shader_feature _REALTIME_MODE
